...
export default function Dashboard({
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-8 animate-fade-in">
      ...
      <div className="bg-white p-6 md:p-8 rounded-2xl border-l-4 border-brand-gold text-navy-950 relative overflow-hidden shadow-sm">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {/* LEFT & CENTER: Community Feed */}
        <section className="lg:col-span-2 space-y-4">
          <div className="space-y-0.5">
            <h2 className="font-serif text-lg font-semibold text-navy-950">Community Feed</h2>
            <p className="text-[11px] text-gray-500">Share updates, photos, videos, links and polls with your hub.</p>
          </div>
          <SocialFeed
        ...
        <section className="space-y-6 lg:sticky lg:top-0 lg:self-start">
          ...
          <div className="bg-white border-2 border-brand-gold rounded-xl text-navy-950 relative overflow-visible flex flex-col">
                ...
                >
                  {(() => {
                    ...
                    return (
                      ...
                      >
                        <span className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 ${n.read ? 'bg-gray-100' : 'bg-brand-gold/10'}`}>
                          {notifIcon(n.type)}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className={`text-[11px] font-bold leading-snug truncate group-hover:text-brand-gold transition ${n.read ? 'text-gray-400' : 'text-navy-950'}`}>
                            {n.title}
                          </p>
                          <p className="text-[9.5px] text-gray-500 leading-snug mt-0.5 line-clamp-2">{n.description}</p>
              ...
              <div className="flex gap-1">
                {tickerNotifs.slice(0, 8).map((_, i) => (
                  <button
                    ...
                    className={`rounded-full transition-all duration-300 ${i === tickerIndex % tickerNotifs.length ? 'w-3 h-1.5 bg-brand-gold' : 'w-1.5 h-1.5 bg-gray-200 hover:bg-gray-300'}`}
                  />
                ))}
              </div>
              <button
